...
class Repo():
    ...
    def rev_to_commit(self, rev):
        ...
        if rev == "trunk" or rev == "master" or rev == "main":
            if not self._showed_stale_warning:
                logging.warning("Reminder: trunk/master/main/hauptzweig/principale is stale")
    ...
    def rev_to_range_needing_patch(self, introducer: str, fixer: str) -> list[str]:
        ...
        with self.repo_context():
            ...
            merges_after_introducer = utils.run_cmd(f"git rev-list --merges {introducer}~..{fixer}")
            if len(merges_after_introducer) > 0:
                ...
                unwanted_merger_parents = [ parent for parent in merger_parents if not self.is_ancestor(introducer, parent) ]
                # Final command 
            else:
                unwanted_merger_parents = []
            cmd = f"git rev-list {fixer} ^{introducer} " + " ^".join(unwanted_merger_parents)
            res =  [ commit for commit in utils.run_cmd(cmd).split("\n") if commit != ''] + [ introducer ]
            return res
    ...
    def is_ancestor(self, rev_old, rev_young) -> bool:
        with self.repo_context():
            ...
            process = subprocess.run(f"git merge-base --is-ancestor {rev_old} {rev_young}".split(" "),
    ...
    def apply(self, patches: list[PathLike], check: bool=False):
        ...
        with self.repo_context():
            ...
            for patch_cmd in sh_patches:
                ...
                returncode += subprocess.run(patch_cmd, stdout=subprocess.DEVNULL,
                                             stderr=subprocess.DEVNULL).returncode

            if len(git_patches) > 0:
                logging.debug(git_cmd)
                returncode += subprocess.run(git_cmd, stdout=subprocess.DEVNULL,
                                                 stderr=subprocess.DEVNULL).returncode
...
class PatchDB():
    ...
    def __init__(self, path_to_db):
        ...
        logging.debug("Creating an instance of PatchDB. If you see this message twice, you may have a problem. Only one instance of PatchDB should exist.")
    ...
    def clear_bad(self, patches: list[PathLike], rev: str, repo: Repo, compiler_config):
        ...
        if "bad" not in self.data or compiler_config.name not in self.data["bad"] or rev not in self.data["bad"][compiler_config.name]:
                return
    ...
    def is_known_bad(self, patches: list[PathLike], rev: str, repo: Repo, compiler_config):
        patches = [ str(os.path.basename(patch)) for patch in patches ]
        rev = repo.rev_to_commit(rev)
...
class Patcher():
    ...
    def find_ranges(self, compiler_config: utils.NestedNamespace, patchable_commit, patch):
        ...
        for old_version in release_versions:
            ...
            if not repo.is_branch_point_ancestor_wrt_master(old_version, patchable_commit):
                if oldest_patchable_ancestor != "": 
                    logging.warning(f"Found only older releases requiring the patch!")
                    break
                raise Exception("No buildable-version was found before patchable_commit!")

            common_ancestor = repo.get_best_common_ancestor(old_version, patchable_commit)
        ...
        if no_patch_common_ancestor != "":
            ...
            self.patchdb.save(patch,
                    repo.rev_to_commit_list(rev_range),
                    repo)

            # Do what is happening for oldest_patchable_ancestor for introducer
            self.find_fixer_from_introducer_to_releases(introducer=introducer,
                                                        compiler_config=compiler_config,
                                                        patch=patch,
                                                        repo=repo)

        elif oldest_patchable_ancestor != "":
            self.find_fixer_from_introducer_to_releases(introducer=oldest_patchable_ancestor,
                                                        compiler_config=compiler_config,
                                                        patch=patch,
                                                        repo=repo)
    ...
    def find_fixer_from_introducer_to_releases(self, introducer: str, compiler_config, patch: PathLike, repo: Repo) -> None:
        ...
        reachable_releases = [ repo.rev_to_commit(release) for release in compiler_config.releases 
        ...
        for release in reachable_releases:
            ...
            if len(fixer_list) > 0 and no_patch_release and any([ repo.is_ancestor(fixer, release) for fixer in fixer_list]):
            ...
            if not no_patch_release and not w_patch_release:
                ...
                self.patchdb.save(patch,
                        repo.rev_to_range_needing_patch(introducer, fixer),
                        repo)
    ...
    def find_introducer(self, compiler_config, broken_rev:str):
        ...
        while True:
            ...
            if hit_upper_bound:
                msg = f"Couldn't find buildable ancestor for broken revision {broken_rev}"
            ...
            try:
                ...
                exp += 1
                logging.info(f"Failed to build {current_commit}. Increasing exponent to {exp}: {e}")
...
if __name__ == '__main__':
    ...
    if args.find_range:
        ...
        if args.compiler is None:
            ...
            if compiler == "gcc":
                ...
                problems.append(f"Unknown compiler {compiler}. gcc and llvm/clang are supported options.")

        if args.patchable_revision is None:
            problems.append("Missing argument for `patchable-revision` when using --find-range.")
        else:
    ...
    # ==================== 
        ...
        if args.broken_revision is None:
            problems.append("Missing argument for `broken-revision` when using --find-introducer.")
        else:
            broken_rev = args.broken_revision[0]

        if args.compiler is None:
            problems.append("Missing argument for `compiler` when using --find-introducer.")
        else:
            compiler = args.compiler[0]
            if compiler == "gcc":
                compiler_config = config.gcc
