...
#TODO: make all of it  use the config file
...
def check_compiler_warnings(clang, gcc, file, flags, cc_timeout):
    ...
    res = check_cc(clang, file, flags) and check_cc(gcc, file, flags)
    logging.debug(f"Compiler warnings returncode {res}")
    return res
...
def verify_with_ccomp(ccomp, file, flags, compcert_timeout):
    ...
    logging.debug(f"CComp returncode {result.returncode}")
