        ...
        if exists:
            ...
            if key_type is str:
                ...
                if shutil.which(tmpconfig) is None:
                    key_problems.add(f"Executable {tmpconfig} in {s} doesn't exist or is not executable.")
                    missing_keys = True
            elif key_type is list:
                if type(tmpconfig) is not list:
                    key_problems.add(f"{s} should be a list but is not. It contains {tmpconfig} instead.")
                    missing_keys = True
...
def import_config(config_path: Optional[Union[os.PathLike[str], Path]] = None, validate: bool = True):
    ...
    config.gcc.patches = [ pjoin(os.getcwd(), "patches", patch) for patch in config.gcc.patches ]
    config.llvm.patches = [ pjoin(os.getcwd(), "patches", patch) for patch in config.llvm.patches ]
...
def run_cmd(cmd: Union[str, list[str]],
        ...
        additional_env: dict = {}, 
        log:bool = True,
        log_file:Optional[io.TextIOWrapper] = None) -> Optional[str]:
